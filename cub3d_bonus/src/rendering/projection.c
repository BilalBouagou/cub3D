...
/*   Updated: 2023/11/28 20:57:03 by bbouagou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_3d_projection(t_data *data, t_ray ray, int x)
{
	...
	if (ray.wall_top > 0)
		draw_line(data, x, 0, x, ray.wall_top, data->textures.ceiling_color);
	draw_walls(data, ray, x);
	if (ray.wall_bottom < WINDOW_HEIGHT)
		draw_line(data, x, ray.wall_bottom, x, WINDOW_HEIGHT, data->textures.floor_color);
}